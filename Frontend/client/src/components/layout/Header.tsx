...
import { Coins, Swords, Shield, LogOut, Trophy, Store } from 'lucide-react';

export const Header = () => {
  const { user } = useAuthStore();

  // Спеціальна функція для стилізації активного посилання в меню
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 px-4 py-1.5 rounded-full shadow-inner">
            ...
            <div className="flex items-center gap-1.5 text-blue-400 font-bold" title="Рівень">
              <Shield size={18} />
              <span className="mt-0.5">{user?.level || 1}</span>
            </div>
          </div>
